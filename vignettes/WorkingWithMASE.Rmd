...
1. **Construction**: Building MASE objects from matrices, existing Bioconductor objects, or files
...
**Prerequisite**: If you are new to MASE, read the **Introduction** vignette
first to understand the MASE structure and components.
...
3. **From existing objects**: Convert SpatialExperiment, SingleCellExperiment, or other objects
4. **Helper functions**: Quick construction for common use cases

## Minimal MASE

The simplest MASE has one experiment, specimen metadata (colData), and a sample
mapping (sampleMap). No spatial elements are required initially.
...
| `subsetByAssay(x, y)` | Assays | All spatial slots |
...
(vector polygons). You can convert between them:
...
cat("4. Store as LabelsLayerList in MASE\n")
...
cat("1. Load label matrix (from file or LabelsLayerList)\n")
...
- Spatial queries (point-in-polygon, overlap)
- Visualization with ggplot2 + sf

# Next steps

- **Use cases**: See the **Use Cases** vignette for realistic workflows with
  data readers and spatial transformations
- **Introduction**: Review MASE structure, design principles, and terminology
